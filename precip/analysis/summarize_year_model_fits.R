...
mfiles <- dir('output/stan_fits', '.*year_effects.RDS', full.names = TRUE)
...
for(i in 1:length(mfiles)){ 
  ...
  if(vr == 'growth') { 
    fit_summary <- summary(temp_fit, c('a', 'b1_mu', 'b1', 'bg', 'sigma', 'sig_a', 'sig_b1', 'w'))$summary
  }
  
  if(vr == 'survival') { 
    fit_summary <- summary(temp_fit, c('a', 'b1_mu', 'b1', 'bg', 'sig_a', 'sig_b1', 'w'))$summary
  }
  
  if(vr == 'recruitment') { 
    fit_summary <- summary(temp_fit, c('a', 'bg', 'sig_a', 'theta', 'u', 'w'))$summary
  }
