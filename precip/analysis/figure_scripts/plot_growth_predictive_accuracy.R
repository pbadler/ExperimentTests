...
mfiles <- dir('output/stan_fits/predictions', '.*growth.*4_predict.RDS', full.names = TRUE)
dffiles <- dir('data/temp_data', '.*growth.*cleaned_dataframe.RDS', full.names = T)
...
for( i in 1:length(mfiles)){ 
  ...
  Y <- scale( df$Y[df$Period == 'Historical'])
  Yhold <- scale( df$Y[df$Period == 'Modern'], attr( Y, 'scaled:center'), attr(Y, 'scaled:scale'))
  df$Y <- c(Y, Yhold)
  ...
  pdf(file.path( 'figures', 'stan_fits',  paste(spp, 'growth_predictive_accuracy.pdf', sep = '_')), height = 8, width = 11)  
