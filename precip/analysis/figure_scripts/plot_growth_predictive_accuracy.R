...
climate_models    <- dir('output/stan_fits', '.*_growth_climate_fit.RDS', full.names = TRUE)
treatment_models  <- dir('output/stan_fits', '.*_growth_treatment_fit.RDS', full.names = T)
year_models       <- dir('output/stan_fits', '.*_growth_year_effects_fit.RDS', full.names = T)

dffile <- dir('data/temp_data', 'modified_growth_data_lists_for_stan.RDS', full.names = T)
...
p1 <- avg_error_per_iteration %>% 
  group_by(species) %>% 
  do(gg = gg_it %+% .  + ggtitle (paste( 'Average error per iteration for', .$species, 'growth')))
...
p2 <- avg_error_per_obs %>% 
  group_by(species) %>% 
  do(gg = gg_obs %+% .  + ggtitle (paste( 'Average error per observation for', .$species, 'growth')))
...
p3 <- avg_error_by_size %>% 
  group_by(species) %>% 
  do(gg = gg_size %+% .  + ggtitle (paste( 'Average error per observation for', .$species, 'growth')))

pdf(file.path( 'figures', 'predictions',  paste('error_in_growth_predictions.pdf', sep = '_')), height = 8, width = 11)  
