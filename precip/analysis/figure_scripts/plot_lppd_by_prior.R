...
# in sample lppd just for fun ------------------------------------------------------------------------------------# 

all_lppds <- read.csv('output/WAIC_scores_compiled.csv')

all_lppds <- all_lppds %>% filter( type == 'in_sample') %>% dplyr::select(-type)
...
gg_no_outlier <- 
  ...
  do(gg = gg_lppd_base %+% . + 
       geom_smooth(aes( group = 1), color = 'blue',  se = FALSE) + 
       ggtitle(paste('in sample lppd by regularization plot for', unique(.$species), unique(.$vital_rate), 'model', unique(.$model)) ))
...
#pdf('figures/in_sample_lppd_by_lambda.pdf', height = 8 , width = 8 )
