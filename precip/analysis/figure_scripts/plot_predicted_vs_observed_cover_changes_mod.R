...
for(i in 1:nrow(grd)){ 
  ...
  overall_MSE <- 
    df %>% ungroup %>% group_by( Period) %>% summarise( MSE = mean((observed-predicted)^2))
  
  overall_cor <- 
    df %>% ungroup %>% group_by( Period ) %>% summarise( cor = cor(predicted, observed))
  ...
  pdf(paste0( 'figures/predictions/', spp, '_', model , '_model_predicted_and_observed_population_growth_rates2.pdf'), height = 8, width = 8)
  print( pts )
  dev.off()
