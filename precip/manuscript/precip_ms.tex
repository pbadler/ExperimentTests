...
\smallskip \textbf{Authorship statement:} ARK and PBA designed the experiment and supervised data collection. AT helped with the design of the data analysis.
\smallskip 
...
\begin{enumerate}
\item Climate is thought to be an important driver of population ecology however it can be difficult to determine exactly which aspects of climate affect populations and at what vital rates.  Tests of whether statistical models fit to climate and population data can accurately predict population responses to natural and experimental climate manipulations have rarely been conducted.   
\item Using long-term observational data collected from the 1920's to the 1950's from a sagebrush steppe plant community, previous research has reported a wide variety of climate effects on four dominant plant species. We tested whether statistical models fit to the historical data could predict the population response of plants in these original plots to natural and experimentally imposed climate variation some 60 years after the historical data collection ended. New plots in the same location where the historical data were collected were assigned to either drought or irrigation treatments imposed using rainfall shelters and automated sprinklers. The original plots were also monitored and served as a control. We modeled survival, growth and recruitment as functions of spring temperature and spring and summer soil moisture. We then used statistical models fitted to the historical data to predict the year and treatment effects observed in contemporary data.  
\item  For survival and recruitment, the treatment effects were never significantly positive, indicating that competitive release was not underestimated by the interspecific effects included in the baseline model. For  \textit{Poa secunda} recruitment, the removal treatment was actually negative, indicating that our baseline model overestimated competitive release.
