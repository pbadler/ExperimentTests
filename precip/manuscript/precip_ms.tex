...
\item Using observational data collected from the 1920's to the 1950's from a sagebrush steppe plant community, previous research has reported a wide variety of climate effects on four dominant plant species. We tested whether statistical models based on these historical data could predict how these species respond to natural and experimentally imposed climate variation from 2006 to 2016, some 60 years after the historical data collection ended. 
\\item We established sixteen new plots in the same location as the historical data and assigned each to either drought or irrigation treatments imposed using rainfall shelters and automatic sprinklers. The original plots were also monitored and served as ambient climate controls. Using the historical data, we fit statistical models for survival, growth and recruitment of each species in response to seasonal temperature and soil moisture.  We used these models to predict variation in species vital rates in the experiment and used a simple individual based population model to predict one year ahead population growth rates in each of the experimental plots.   
\item Over the course of the experiment, average plot cover of \textit{Hesperostipa comata} and \textit{Pseudoroegneria spicata} declined, while cover of \textit{Poa secunda} showed a trend towards increase in the irrigated plots. At the level of individual vital rates, experimental drought led to decreases in \textit{} growth, survival, while irrigation led to increases in \textit{}. 
\item Vital rate models fit to the historical data correctly predicted significant increases in X and X and significant decreaes in X and X.  However, the historical models predicted significant changes in X X and X that were not observed in the experiment.  Moreover, the experiment led to a significant change in X that was not predicted by the historical data. 
