...
if(trtEffects==F){
 outfile1=paste("simulations/",qName,"_ARTRcov_removals_noTrt.csv",sep="")
 #outfile2=paste("simulations/",qName,"_validation_den_removals_noTrt.csv",sep="")
}else{
 outfile1=paste("simulations/",qName,"_ARTRcov_removals_Trt.csv",sep="")
...
# matplot(predA[,1:4],type="o",
#   col=myCol,lty=c(rep("solid",Nspp),c(rep("dashed",Nspp))),
#   pch=c(rep(16,Nspp),rep(1,Nspp)),xlab="Year",ylab="Cover")
# matplot(predN[,1:4],type="o",
#   col=myCol,lty=c(rep("solid",Nspp),c(rep("dashed",Nspp))),
#   pch=c(rep(16,Nspp),rep(1,Nspp)),xlab="Year",ylab="Density")
# 
# write.table(predA,paste0("ibm/",outfile1),row.names=F,sep=",")
# write.table(predN,paste0("ibm/",outfile2),row.names=F,sep=",")

# extract Artemisia cover
ARTRcov = bigA[,1,]
write.table(ARTRcov,paste0("ibm/",outfile1),row.names=F,sep=",")
