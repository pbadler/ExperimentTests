...
# simplest model
m0 <- glmer(survives~logarea+ W.ARTR + W.HECO + W.POSE + W.PSSP +(logarea|year),data=allD,family=binomial,control=glmerControl(optimizer="bobyqa")) 

# put indicators on intercept only
m1a <- glmer(survives~logarea + Treatment + W.ARTR +(logarea|year),data=allD,family=binomial,control=glmerControl(optimizer="bobyqa")) 
m1b <- glmer(survives~logarea + Treatment2 + W.ARTR +(logarea|year),data=allD,family=binomial,control=glmerControl(optimizer="bobyqa")) 
m1c <- glmer(survives~logarea + Treatment3 + W.ARTR +(logarea|year),data=allD,family=binomial,control=glmerControl(optimizer="bobyqa")) 
