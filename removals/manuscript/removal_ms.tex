...
expected size on arithmetic scale is not exp(expected log size), because of Jensen's inequality). 
...
However, \new{when we tested the effects of removal on cover by analyzing year-to-year changes in (log) cover, a measure of population 
growth rate, none of the removal treatment effects were significantly different from zero (Table \ref{table:coefficients}). For \textit{P. spicata}, 
...
Fitting with weighted least squares to address heteroscedasticity in residuals indicated even less evidence for a positive 
response of  \textit{P. spicata} to removal.} 
\comment{SPE: I think you need to explain how did ``analyzing year-to-year changes in (log) cover'' -- what was the response, what were the
covariates? I think reviewers will also want to know what was done. And without that information, 
it's not clear what the residuals would be, or what weighted least squares would entail. } 
However, these treatment comparisons do not directly address our questions about whether 
responses to removal were adequately predicted by models fit to historical, observational data. \comment{SPE: is it ``historical, observational data'' or was the ``baseline'' model
fitted to observational data both historical and modern?} 
...
\new{SPE: As in our previous studies using only historical data, the neighborhood competition coefficients in the 
...
removals either larger or smaller than the effect predicted by a model based on data from the historical control plots. 
\comment{SPE: is the baseline model just based on the historical data, or also on the modern data from control plots? 
For me, ``historical control plots'' is ambiguous on this point.} 
...
from control plots. For the growth models, the 95\% confidence intervals on the removal effects were positive for \textit{P. secunda}  and \textit{P. spicata},
 ...
 (Fig. \ref{fig:VitalRateTest}; Tables \ref{ARTRgrowth} to \ref{PSSPgrowth}). In contrast, \textit{Poa secunda} had lower recruitment in removal 
...
%Our population models based on historical, observational data reveal weak interspecific interactions \citep{adler_coexistence_2010,chu_large_2015} and, as a result, 
%predict very little competitive release among the four dominant species in a sagebrush steppe. Despite the fact that previous removal studies have reported variable 
%results, ranging from negative effects of removal on remaining species to complete competitive release, we were suspicious of our model's \new{ability to accurately 
%describe interspecific interactions, so we conducted a removal experiment to test it.} The results provide substantial support for our model's predictions.
...
The results provide substantial support for our model's predictions, and for the weakness of interspecific interactions in this community. \comment{SPE has
revised this paragraph. The previous version is in the tex file, commented out.}
...
short (Fig. \ref{fig:IBM1step}) and long-term (Fig. \ref{fig:IPMresults}) dynamics for \textit{H. comata} and \textit{A. tripartita}. 
...
\new{How can we reconcile the insensitivity of grasses to sagebrush removal in our study with evidence from rangeland ecology 
...
PBA and ARK designed the experiment and supervised data collection with the help of JBT; PBA, SPE, GH and BT analyzed data \new{and 
...
The one exception to this pattern is the effect of ARTR on recruitment by POSE, but the nonconforming
coefficient ($\omega$[3,1] = -0.6589 in Table \ref{table:recruitment}) has low imprecision (sd = 0.3342, Table \ref{table:recruitment}.} 
\label{table:alphas}
\begin{tabular}{lrrrr}
       & ARTR &  HECO &  POSE &  PSSP \\
\hline 
...
ARTR  & -0.5881 & 0.0635 & 0.0220 & 0.1164 \\
HECO   &  -0.4800 & -1.7368& 0.0388 &  -0.3682\\
POSE  &  -0.6589& -0.1021& -1.8943 &   -0.1953\\
PSSP  & -0.1867 &-0.3746  & 0.1131& -1.7379\\
\hline
\end{tabular}
\end{table}
