...
\new{Note that the ``baseline" and ``removal" models are not estimated separately; we estimate all parameters together, but when we generate predictions from the baseline model, we simply set the removal treatment effects to zero. We chose this over an alternative approach of fitting a model only to data from control plots and then making and evaluating predictions for the removal plots. The principal advantage of our approach is that the significance test (confidence intervals) for the removal effect is well-defined; we do not know of valid tests for fitting on one data set and testing on another (GILES??). In addition, the alternative approach would return essentially the same results because adding data from the removal plots had negligible effects on other model parameters. Correlations between the parameters of a model fit only to control plot data and our full model (leaving aside the removal effects) were stronger than 0.999. Why don't we find strong interactions between removal treatments and the estimated competition coefficients? The answer is that because our treatments involved complete, not just partial, removal of target species, their effects can be captured with an intercept term, as illustrated in Fig. \ref{fig:PSSPresids}. As a result, if, for example, one of the grasses responds more positively to removal of \textit{A. tripartita} than our baseline model predicts, we cannot determine whether this reflects a release from competition (i.e. a poorly estimated competition coefficient) or some other effect of our removal treatment  independent of competitor densities.   }
...
\new{Although all the parameters in our model are estimated with uncertainty, we chose not to propage parameter uncertainty through the 
...
Our previously published results showed that interspecific competition is weak relative to the effects of interannual environmental variability 
\citep{Tredennick2017}. If we included parameter uncertainty along with random year effects, we would be conflating parameter uncertainty 
...
is larger than the observationally-based model predicts. Therefore, we used estimated means of all parameters, including those with confidence intervals that overlapped zero. Based on the same rationale, we used a deterministic version of the model (e.g., 
...
gave us a very real opportunity to be publicly wrong. Second, testing an observationally-based model with an experimental manipulation, 
...
Future work in other systems might benefit from this insight. Second, the success of our models supports the use of long-term observational data for characterizing competitive interactions. We see no reason why this approach would not work just as well in a community with stronger 
interspecific interactions. After all, our approach did detect strong intraspecific effects. 
Finally, our results give cause for optimism about population forecasting.
\citet{Crone2013} found that matrix projection models for plants failed to forecast population trends due environmental differences 
between the data collection and forecast periods. Our population models performed quite well after accounting for year-to-year 
environmental variation with random year effects. }
...
Funding was provided by NSF grants DEB-1353078 and DEB-1054040 to PBA and DEB-1353039 to SPE and GH, and by the Utah Agricultural Experiment Station, which approves this contribution as journal paper number 9033. 
